...
def extract_text_from_pdf(pdf_file) -> str:
    ...
    doc = fitz.open(stream=pdf_file.read(), filetype="pdf")  # Open PDF
    text = ""
    for page in doc:
...
def chunk_text(text: str, max_tokens: int = 4000) -> list:
    """Split text into chunks of approximately max_tokens size at sentence boundaries."""
    ...
    for sentence in sentences:
        sentence_length = len(sentence) + 1  # Adding 1 for space
...
def summarize_chunk(chunk: str) -> str:
    """Summarize a single chunk while handling errors."""
    prompt_template = """Analyze this portion of a legal document and provide:

1. Key points covered in this section
2. Important requirements or conditions mentioned
3. Any notable exceptions or special cases
...
{text}"""
    ...
    try:
        ...
        response = llm.invoke(prompt)  # Replace with actual API call
        ...
        print("Waiting for 65 seconds before retrying...")
        time.sleep(65)
        return ""

def summarize_text(text: str) -> str:
    """Summarizes a legal document in multiple stages to avoid exceeding API limits."""
    if not text.strip():
    ...
    # If summaries are too large, summarize them in smaller batches
    while len(" ".join(summaries)) > 4000 * 3.5:  # If summaries exceed ~4000 tokens, re-chunk
        print("Summarized text too large, re-summarizing in smaller batches...")
        summaries = [summarize_chunk(" ".join(summaries[i:i+2])) for i in range(0, len(summaries), 2)]
        summaries = [s for s in summaries if s]  # Remove empty responses

    # Final summarization request
    final_prompt = """Please analyze the following legal document summaries and write a comprehensive final summary that is specific to the content:

1. Document Overview (2-3 paragraphs) based on actual document type, parties, and purpose.
2. Section-by-Section Analysis using actual section details and clauses.
3. Key Takeaways (3-5 specific points about risks, obligations, etc.)
...
"""
    ...
    try:
        final_response = llm.invoke(final_prompt.format(text="\n\n".join(summaries)))
...
- Answer in hinglish only if the question is in hinglish.
...
st.sidebar.header("Upload a PDF to Summarize")
